...
class MarketData:
    ...
    timestamp: datetime
    regime: str = "UNKNOWN"
    m1_candles: Optional[CandleArray] = None
    preprocessed: Optional[dict] = None
    data_fidelity_score: float = 1.0
...
class BaseStrategy(ABC):
    ...
    def get_execution_id(self, timestamp: datetime) -> str:
        return f"{self.strategy_id}_{timestamp.isoformat()}_{uuid.uuid4().hex[:8]}"
    ...
    def on_candle(self, market_data: MarketData) -> Optional[TradeSignal]:
