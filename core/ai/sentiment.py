...
class SentimentVetter:
    ...
    def vet_signal(self, symbol: str, direction: str, news_events: list) -> dict:
        ...
        Retries up to 3 times with exponential back-off on transient API failures.
        On all retries exhausted, returns advisory bypass (approved=True) so a
        temporary API outage does NOT halt trading.
        """
        ...
        # Build the prompt once, outside the retry loop
        news_summary = "\n".join([
        ...
        for attempt in range(1, max_retries + 1):
            try:
                response = requests.post(
                    self.endpoint, headers=headers, json=payload, timeout=10
                )
                ...
                logger.warning(f"[Sentiment] Attempt {attempt}/{max_retries} failed: {last_error}")

            except Exception as e:
                last_error = str(e)
                logger.warning(f"[Sentiment] Attempt {attempt}/{max_retries} exception: {e}")

            if attempt < max_retries:
                _time.sleep(backoff)
                backoff *= 2.0

        # All retries exhausted: advisory bypass (do not veto on API failure)
        logger.error(f"[Sentiment] All {max_retries} attempts failed. Advisory bypass. Last error: {last_error}")
        return {"approved": True, "confidence": 0.5, "reason": "API_RETRY_EXHAUSTED"}
